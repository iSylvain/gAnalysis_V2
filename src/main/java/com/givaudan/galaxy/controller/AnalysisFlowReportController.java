...
import com.givaudan.galaxy.service.SynthesisClientService;
...
@Named
@ViewScoped
public class AnalysisFlowReportController implements Serializable {

    private static final Logger logger = Logger.getLogger(AnalysisFlowReportController.class.getName());
    ...
    public void load(Long id) throws ServletException, IOException {
        ...
        //récupération de l'image de la synthèse dans gSynthesis
        SynthesisClientService service = new SynthesisClientService();
        SynthesisData synthesisData = service.getSynthesisData(instance.getProcessCode(), instance.getOperation());
        if (synthesisData != null) {
            logger.log(Level.SEVERE, "synthesisData wired");
        } else {
            logger.log(Level.SEVERE, "synthesisData not wired");
        }
...
//            response.setHeader("Content-disposition", "attachment;filename=\"" + fileName + "\"");
            ...
            reportUtil.createAnaylsisFlowPDF(writer, document, instance, synthesisData, context);

            //add analysis
            if (instance.isAnalysisForFlowListWired()) {
                for (AnalysisForFlow a : instance.getAnalysisForFlowList()) {
                    document.newPage();
                    if (a.getAnalysis().getTypeOfAnalysis().contains("gc")) {
                        try {
                            CriteriaBuilder cb = entityManagerDAO.getEntityManager().getCriteriaBuilder();
                            CriteriaQuery<GC> query = cb.createQuery(GC.class);
                            Root<GC> object = query.from(GC.class);
                            Fetch<GC, AnalysisStandard> standards = object.fetch("standards", JoinType.LEFT);
                            Fetch<GC, AnalysisReferenceData> reference = object.fetch("referenceData", JoinType.LEFT);
                            query.where(cb.equal(object.get("id"), a.getAnalysis().getId()));
                            GC gc = entityManagerDAO.getEntityManager().createQuery(query).getSingleResult();
                            
//                            //fetch 
//                            gc.getStandards().size();
//                            gc.getReferenceData().getId();
                            
                            reportUtil.createAnaylsisGCPDF(writer, document, (GC) gc, context);
                        } catch (NoResultException | NonUniqueResultException nre) {
                            logger.log(Level.SEVERE, "Error findById : {0}", nre.getMessage());
                            throw new NoEntityException("No entity with id : " + id);
                        }
                    } else {
                        reportUtil.createAnaylsisPDF(writer, document, a.getAnalysis(), context);
                    }
                }
            }
